...
class _DeliveryReportTabState extends ConsumerState<DeliveryReportTab> {
  ...
  Future<void> _exportExcel(List<Map<String, dynamic>> data) async {
    ...
    final bytes = ex.save();
    if (bytes != null) {
      saveExcel(bytes, 'laporan_pengiriman.xlsx');
      if (mounted) {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: data.isEmpty
              ? const Center(
                  child: Text('Belum ada data pengiriman',
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                              flex: 2,
                              child: Text(
                                  'Rp ${_formatPrice((row['total_amount'] ?? 0).toDouble())}',
                                  style: const TextStyle(
                                      color: AppColors.neonGreen,
                                      fontSize: 11))),
                          Expanded(
                              flex: 2,
                              child: Text(_formatDate(row['created_at'] ?? ''),
                                  style: const TextStyle(
