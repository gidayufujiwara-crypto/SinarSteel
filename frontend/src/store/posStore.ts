...
export const usePosStore = create<PosState>((set, get) => ({
  ...
  updateQty: (produk_id, qty) => {
    if (qty <= 0) { get().removeFromCart(produk_id); return }
...
function generateStruk(trx: any, cart: CartItem[], total: number, diskon: number, total_after: number): string {
  ...
  if (trx.bayar) lines.push(`Uang     : Rp ${Number(trx.bayar).toLocaleString()}`)
  if (trx.kembalian !== null) lines.push(`Kembali  : Rp ${Number(trx.kembalian).toLocaleString()}`)
  lines.push('=============================')
